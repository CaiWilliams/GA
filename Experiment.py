...
class Experiment:
    ...
    def __init__(self, experiment_name = "Ongoing_Experiment", base_experiment='sim.json', population_N = 500, chromosomes_N = 4, chromosomes_min = [10e-9, 10e-9, 10e-9, 10e-9], chromosomes_max = [1e-6, 1e-6, 1e-6, 1e-6], chromosomes_start = [1e-7, 1e-7, 2.2e-7, 1e-7], mutation_chance= 0.2, best_frac = 0.1, worst_frac = 0.2, max_generations = 100, objective_function = PCE, target = 100,  **objective_function_arguments):
        ...
        while self.generation <= max_generations:
            ...
            with open(os.path.join(os.getcwd(),'Experiment_Status','Experiment-Status', experiment_name + '.exp'),'wb') as f:
    ...
    def define_population(self):
        P = Population(self.base_experiment, self.population_N, self.mutation_chance)
...
#Experiment(population_N=5,base_experiment='PM6Y6.json',max_generations=3,objective_function=PCE)