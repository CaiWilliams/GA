...
with open('OnGoing_Experiment.exp', 'rb') as handle:
    ...
    average = [np.std(i.result[:,0]) for i in data]
    min = [np.min(i.result[:,0]) for i in data]
    max = [np.max(i.result[:,0]) for i in data]
    #print(np.argmax(data[-1].result))
    print(np.std(data[-1].result[:,1]))
    print(np.std(data[-1].result[:, 0]))
    print(np.std(data[-1].result[:, 2]))
    plt.plot(average)
    plt.fill_between(range(len(data)), min, max, alpha=0.2)
    plt.xlabel('Generations')
    plt.ylabel('Power Conversion Efficiency for Cost (%/£)')
    plt.show()

    average = [np.std(i.result[:,1]) for i in data]
    ...
    average = [np.std([m.chromosomes[0] for m in i.population]) for i in data]
    ...
    average = [np.std([m.chromosomes[1] for m in i.population]) for i in data]
    ...
    average = [np.std([m.chromosomes[2] for m in i.population]) for i in data]
    ...
    average = [np.std([m.chromosomes[3] for m in i.population]) for i in data]
